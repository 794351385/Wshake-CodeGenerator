...
import com.wshake.generator.utils.sql.SqlInjectionUtils;
import com.wshake.generator.utils.sql.StringEscape;
import com.wshake.generator.utils.support.BiIntFunction;
...
public final class StringUtils {
    ...
    public static Boolean startsWith(String str, String suffix) {
        return startsWith(str, suffix,false);
    }

    /**
     * Check if a String ends with a specified suffix (optionally case
     * insensitive).
     * 检查字符串是否以指定的后缀结束(可选不区分大小写)。
     *
     * @param str        the String to check, may be null
     * @param suffix     the suffix to find, may be null
    ...
    private static Boolean startsWith(String str, String suffix, Boolean ignoreCase) {
        if (str == null || suffix == null) {
            return (str == null && suffix == null);
        }
        if (suffix.length() > str.length()) {
     ...
     * @return ignore
     */
    public static String removeSuffixAfterPrefixToLower(String rawString, int index) {
        return prefixToLower(rawString.substring(0,rawString.length()-index), 1);
    }
    ...
    public static String removePrefix(String str,String prefix) {
        for (String pf : prefix.split(",")){
            if(StringUtils.startsWith(str,pf)){
                str = StringUtils.removePrefixAfterPrefixToLower(str,pf.length());
            }
        }
        return str;
    }
     ...
     * @return String
     */
    public static String removeSuffixes(String str,String suffix) {
        for (String pf : suffix.split(",")){
            if(StringUtils.endsWith(str,pf)){
                str = StringUtils.removeSuffixAfterPrefixToLower(str,pf.length());
            }
        }
        return str;
    }
