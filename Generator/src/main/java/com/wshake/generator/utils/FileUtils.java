...
public class FileUtils {
     ...
     * 路径相同返回 ""
     * 路径不相同
     *  baseDir的父目录为空 返回file的全目录截取到baseDir的长度
     * 最后返回
     *  baseDir的父目录不为空 返回file的全目录截取到baseDir的长度+1
     */
     ...
     * 递归查询某个目录下的所有文件
     */
    public static void searchFiles(File dir, List<File> files) {
        if (dir.isDirectory()) {
            File[] subFiles = dir.listFiles();
            if (subFiles != null) {
                for (File subFile : subFiles) {
                    if (subFile.isDirectory()) {
                        searchFiles(subFile, files);
                    } else {
                        files.add(subFile);
                    }
                }
